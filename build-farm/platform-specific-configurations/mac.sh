...
# If using Xcode >= 15, then Apple have introduced linker option -deterministic, to enable reproducible dylib's
if [ $(xcodebuild -version | grep "Xcode" | awk '{print $2}' | cut -d. -f1) -ge 15 ]; then
