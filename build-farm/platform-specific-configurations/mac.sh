...
if [ ! -d "$(eval echo "\$$BOOT_JDK_VARIABLE")" ]; then
  ...
  if [ ! -x "$bootDir/Contents/Home/bin/javac" ]; then
    ...
    elif [[ ("$JDK_BOOT_VERSION" -eq 17) && ( -x "/Library/Java/JavaVirtualMachines/temurin-17.jdk/Contents/Home/bin/javac") ]]; then
        export "${BOOT_JDK_VARIABLE}"="/Library/Java/JavaVirtualMachines/temurin-17.jdk/Contents/Home"
