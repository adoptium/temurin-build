...
# This function creates a temporary clone of adoptium_mirror_repo and
...
#
# For Adoptium mirror the merged upstream OpenJDK commit being built is for the various mirror branches :
# - release branch : Direct parent should be the openjdk "jdk-" tag
# - dev branch     : Direct parent or grandparent should be the openjdk "jdk-" tag
# - For a jdk(HEAD) repo dev branch : Openjdk commit will be the direct parent commit that is not a merge commit
aaafind_openjdk_tagged_parent() {
    local adoptiumMirrorRepo="$1"
    local start_commit="$2"

    local openjdkCommitSHA=""
    ...
    cd adoptium_mirror_repo
  
    # Verify the commit exists
    if ! git rev-parse --verify "$start_commit^{commit}" >/dev/null 2>&1; then
        echo "Error: Invalid commit SHA: $start_commit" >&2
        cd .. && rm -rf adoptium_mirror_repo
        return ""
    fi
    
    # Function to check if a commit has a jdk-* tag (excluding _adopt)
    check_jdk_tag() {
        local commit="$1"
        local tags=$(git tag --points-at "$commit" 2>/dev/null)
        # Look for tags starting with "jdk-" but NOT ending with "_adopt"
        if echo "$tags" | grep "^jdk-" | grep -v "_adopt$" | grep -q .; then
            return 0
        fi
        return 1
    }
    
    # First, check direct parents of the commit
    local parent_num=1
    while [ -z "$openjdkCommitSHA" ]; do
        local parent=$(git rev-parse --verify "$start_commit^$parent_num" 2>/dev/null)
        ...
        # Check if this parent has a jdk-* tag
        if check_jdk_tag "$parent"; then
            openjdkCommitSHA="$parent"
            break
        fi
    ...
    # If no direct parent has a jdk-* tag, check grandparents (one level deeper)
    # This handles the case where HEAD of mirror has a commit between merge and tagged commit
    parent_num=1
    while [ -z "$openjdkCommitSHA" ]; do
        local parent=$(git rev-parse --verify "$start_commit^$parent_num" 2>/dev/null)
        ...
        # Check this parent's parents (grandparents of start_commit)
        local grandparent_num=1
        while [ -z "$openjdkCommitSHA" ]; do
            local grandparent=$(git rev-parse --verify "$parent^$grandparent_num" 2>/dev/null)
            
            # No more grandparents for this parent
            if [ -z "$grandparent" ]; then
                break
            fi
            
            # Check if this grandparent has a jdk-* tag
            if check_jdk_tag "$grandparent"; then
                openjdkCommitSHA="$grandparent"
                break
            fi
            
            grandparent_num=$((grandparent_num + 1))
        done
        
        parent_num=$((parent_num + 1))
    done
    
    # Level 3: If still no jdk-* tag found, pick the direct parent that is NOT a merge commit
    # This handles edge cases where neither parents nor grandparents have jdk-* tags,
    # which is the case for the openjdk/jdk head repo.
    parent_num=1
    while [ -z "$openjdkCommitSHA" ]; do
        local parent=$(git rev-parse --verify "$start_commit^$parent_num" 2>/dev/null)
        ...
        # Check if this parent is NOT a merge commit (has only one parent)
        local parent_parent_count=$(git rev-parse --verify "$parent^2" 2>/dev/null && echo "merge" || echo "non-merge")
        
        if [ "$parent_parent_count" = "non-merge" ]; then
            openjdkCommitSHA="$parent"
            break
        fi
